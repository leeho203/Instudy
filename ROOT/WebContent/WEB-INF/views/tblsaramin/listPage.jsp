...
<title>Tbl Saramin List Page</title>
...
<script>
		$(document).ready(function(){
			...
			$("#searchBtn").click(function(){
				self.location="/tblsaramin/listPage"
			...
			$("#newBtn").click(function(){
				self.location="/tblsaramin/regist";
			});
			...
			<div class="col-sm-9">
				...
				<div class="inbox">
					<table class="table table-condensed table-hover" style="margin:0px;">
						...
						<tbody>
							<c:forEach items="${list}" var="saraminVO">
								<tr style="height:40px;">
									<!-- <td>saraminVO.id</td> -->
									<td align="left" style="vertical-align:middle;">${saraminVO.companyName}</td>
									<td align='left' style="vertical-align:middle; word-break:break-all;">
										<a href='/tblsaramin/read${pageMaker.makeSearchQuery(pageMaker.pageVO.page)}&id=${saraminVO.id}'>${saraminVO.positionTitle}</a>
										<span style='color:red;'><b>[${saraminVO.replycnt}]</b></span>	
									</td>
									<td style="vertical-align:middle;">
										<fmt:formatDate pattern="MM/dd" value="${saraminVO.posting}"/><br>
										<fmt:formatDate pattern="HH:mm" value="${saraminVO.posting}"/>
									</td>
									<!-- 
									<td>	
										<fmt:formatDate pattern="MM/dd" value="${saraminVO.opening}"/>
									</td>
									 -->
									<td style="vertical-align:middle;">	
										<fmt:formatDate pattern="MM/dd" value="${saraminVO.expiration}"/><br>
										<fmt:formatDate pattern="HH:mm" value="${saraminVO.expiration}"/>
									</td>
									<td align="right" style="vertical-align:middle;">${saraminVO.viewcnt}</td>	
								</tr>
							</c:forEach>
						</tbody>
					</table>				
				...
				<div class="inbox" style="margin-top:10px;">
					...
					<div class="btn-group">
						...
						<c:if test='${not empty loginAdmin}'>
							<button class="btn btn-sm btn-default" id="newBtn">DB가져오기</button>
						</c:if>
					</div>
				</div>
							...
							<c:forEach items="${bestList}" var="bestList">
								<tr style="height:35px; border-bottom:1px solid #f0f0f0;">
									<td align='left' style="vertical-align:middle; padding:5px 0px 5px 10px;">
										<a href='/tblsaramin/read?id=${bestList.id}'>
											${bestList.positionTitle}
										</a>
									</td>
									<td align="right" style="vertical-align:middle; padding:5px 10px 5px 0px;">${bestList.companyName}</td>	
								</tr>
							</c:forEach>
					</table>	
				</div>
