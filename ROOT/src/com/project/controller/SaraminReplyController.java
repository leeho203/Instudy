...
import com.project.service.SaraminReplyService;

@RestController
@RequestMapping("/saraminreply")
public class SaraminReplyController {

	@Inject
	SaraminReplyService saraminReplyService;
	...
	public ResponseEntity<String> register(@RequestBody ReplyVO vo){
		...
		try{
			saraminReplyService.registReply(vo);
	...
	public ResponseEntity<List<ReplyVO>> getReplyAll(@PathVariable("bno") Integer bno){
		...
		try{
			List<ReplyVO> list=saraminReplyService.getReplyAll(bno);
	...
	public ResponseEntity<Map<String, Object>> getReplyPage(@PathVariable("bno") Integer bno,
		...
		try{
			...
			vo.setPage(page);
			List<ReplyVO> list=saraminReplyService.getReplyPage(bno, vo);
			
			/*		bno를 이용해서 해당 게시물의 댓글 개수 얻어오고 PageVO객체와 함께 pageMaker를 만들어줌		*/
			int replyCount=saraminReplyService.totalReplyCount(bno);
	...
	public ResponseEntity<String> modifyReply(@PathVariable("rno") Integer rno, 
		...
		try{
			vo.setRno(rno);
			saraminReplyService.modifyReply(vo);
			entity=new ResponseEntity<String>("SUCCESS", HttpStatus.OK);
		}catch(Exception e){
			e.printStackTrace();
			entity=new ResponseEntity<String>(e.getMessage(), HttpStatus.BAD_REQUEST);
	...
	public ResponseEntity<String> removeReply(@PathVariable("rno") Integer rno){
		...
		try{
			saraminReplyService.removeReply(rno);
			entity=new ResponseEntity<String>("SUCCESS", HttpStatus.OK);
		}catch(Exception e){
			e.printStackTrace();
			entity=new ResponseEntity<String>(e.getMessage(), HttpStatus.BAD_REQUEST);
