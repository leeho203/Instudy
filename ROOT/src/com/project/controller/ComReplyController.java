...
public class ComReplyController {
	...
	public ResponseEntity<String> register(@RequestBody ReplyVO vo){
		...
		try{
			comReplyService.registReply(vo);
	...
	public ResponseEntity<List<ReplyVO>> getReplyAll(@PathVariable("bno") Integer bno){
		...
		try{
			List<ReplyVO> list=comReplyService.getReplyAll(bno);
	...
	public ResponseEntity<Map<String, Object>> getReplyPage(@PathVariable("bno") Integer bno,
		...
		try{
			...
			vo.setPage(page);
			List<ReplyVO> list=comReplyService.getReplyPage(bno, vo);
			
			/*		bno를 이용해서 해당 게시물의 댓글 개수 얻어오고 PageVO객체와 함께 pageMaker를 만들어줌		*/
			int replyCount=comReplyService.totalReplyCount(bno);
	...
	public ResponseEntity<String> modifyReply(@PathVariable("rno") Integer rno, 
		...
		try{
			vo.setRno(rno);
			comReplyService.modifyReply(vo);
			entity=new ResponseEntity<String>("SUCCESS", HttpStatus.OK);
		}catch(Exception e){
			e.printStackTrace();
			entity=new ResponseEntity<String>(e.getMessage(), HttpStatus.BAD_REQUEST);
	...
	public ResponseEntity<String> removeReply(@PathVariable("rno") Integer rno){
		...
		try{
			comReplyService.removeReply(rno);
			entity=new ResponseEntity<String>("SUCCESS", HttpStatus.OK);
		}catch(Exception e){
			e.printStackTrace();
			entity=new ResponseEntity<String>(e.getMessage(), HttpStatus.BAD_REQUEST);
